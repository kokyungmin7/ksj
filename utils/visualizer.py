...
def save_visualization(
    ...
    attn_full = trace["attn_map_full"]       # (H, W) float
    crop_img = trace["crop"]
    bbox = trace["bbox"]                     # (x1, y1, x2, y2)
    route = trace["route"]
    ...
    # ── Grid layout: 4 image panels on top, text below ──────────────────────
    gs = fig.add_gridspec(
        2, 4,
        height_ratios=[3, 1],
    ...
    _panel_style = dict(fontsize=9, color="white", fontweight="bold",
                        ha="center", va="top", transform=None)
